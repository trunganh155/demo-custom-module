...
function App() {
  ...
  const settingDayBoCong = (dayBoCong) => {
    dayBoCong.position.z = 0 * -1;

    dayBoCong.position.y = 0;

    dayBoCong.position.x = DDTHau;
  ...
  const settingNepNocBoCong = (nepNocBoCong) => {
    nepNocBoCong.position.z = 0 * -1;

    nepNocBoCong.position.x = 0;

    nepNocBoCong.position.y = height - caoNep;
    ...
    nepNocBoCong.scale.set(1, 1, 1);
    let boundingBoxChanTruoc = new THREE.Box3().setFromObject(nepNocBoCong);
    const sizeChanTruoc = new THREE.Vector3();
    boundingBoxChanTruoc.getSize(sizeChanTruoc);

    nepNocBoCong.scale.x = lenX / sizeChanTruoc.x;
    nepNocBoCong.scale.y = lenY / sizeChanTruoc.y;
    nepNocBoCong.scale.z = lenZ / sizeChanTruoc.z;
  };
  ...
  useEffect(() => {
    ...
    glftLoader.load('/glb/TUGOCBOCONGTRAI.glb', (gltfScene) => {
  ...
  useEffect(() => {
    if (display && gltfUuid) {
      ...
      const dayBoCong = md.getObjectByName('DAY-BO-CONG');
      const nocBoCong = md.getObjectByName('NOC-BO-CONG');
      const hau = md.getObjectByName('HAU');
      const bPhai = md.getObjectByName('BIA-TRAI');
      const chanBoCong = md.getObjectByName('CHAN-BO-CONG');
      ...
      dayBoCong && settingDayBoCong(dayBoCong);
      bPhai && settingBiaPhai(bPhai);
      chanBoCong && settingNepNocBoCong(chanBoCong);
