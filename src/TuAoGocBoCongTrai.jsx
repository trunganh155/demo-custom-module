...
function App() {
  ...
  const settingChanBoCong = (chanBoCong) => {
    chanBoCong.position.z = 0 * -1;

    chanBoCong.position.x = DDTHau;

    chanBoCong.position.y = 0;
    ...
    chanBoCong.scale.set(1, 1, 1);
    let boundingBoxChanTruoc = new THREE.Box3().setFromObject(chanBoCong);
    const sizeChanTruoc = new THREE.Vector3();
    boundingBoxChanTruoc.getSize(sizeChanTruoc);

    chanBoCong.scale.x = lenX / sizeChanTruoc.x;
    chanBoCong.scale.y = lenY / sizeChanTruoc.y;
    chanBoCong.scale.z = lenZ / sizeChanTruoc.z;
  };
  ...
  useEffect(() => {
    if (display && gltfUuid) {
      ...
      chanBoCong && settingChanBoCong(chanBoCong);
