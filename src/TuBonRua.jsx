...
function App() {
  ...
  const [height, setHeight] = useState(0.87);
  ...
  const settingNep1 = (nep1) => {
    ...
    const lenX = DDTBia;
    const lenY = caoChan + ((height - caoChan - DDTNoc - caoNep) / 2) * 0;
  ...
  const settingNep2 = (nep2) => {
    ...
    nep2.position.y = caoChan + ((height - caoChan - DDTNoc - caoNep) / 2) * 1;
  ...
  const settingNep3 = (nep3) => {
    ...
    nep3.position.y = caoChan + ((height - caoChan - DDTNoc - caoNep) / 2) * 2;
  ...
  const settingNepNoc1 = (nepNoc1) => {
    nepNoc1.position.z = DDTBia * -1;
    nepNoc1.position.x = depth - caoNep;
    nepNoc1.position.y = height - DDTBia;

    const lenZ = width - 2 * DDTBia;
    const lenX = caoNep;
    const lenY = DDTBia;

    nepNoc1.scale.set(1, 1, 1);
    let boundingBoxNoc = new THREE.Box3().setFromObject(nepNoc1);
    const sizeNoc = new THREE.Vector3();
    boundingBoxNoc.getSize(sizeNoc);

    nepNoc1.scale.x = lenX / sizeNoc.x;
    nepNoc1.scale.y = lenY / sizeNoc.y;
    nepNoc1.scale.z = lenZ / sizeNoc.z;
  };

  const settingNepNoc2 = (nepNoc2) => {
    nepNoc2.position.z = DDTBia * -1;
    nepNoc2.position.x = DDTHau;
    nepNoc2.position.y = height - DDTBia;

    const lenZ = width - 2 * DDTBia;
    const lenX = caoNep;
    const lenY = DDTBia;

    nepNoc2.scale.set(1, 1, 1);
    let boundingBoxNoc = new THREE.Box3().setFromObject(nepNoc2);
    const sizeNoc = new THREE.Vector3();
    boundingBoxNoc.getSize(sizeNoc);

    nepNoc2.scale.x = lenX / sizeNoc.x;
    nepNoc2.scale.y = lenY / sizeNoc.y;
    nepNoc2.scale.z = lenZ / sizeNoc.z;
  };
  ...
  useEffect(() => {
    ...
    glftLoader.load('/glb/TU-BON-RUA.glb', (gltfScene) => {
  ...
  useEffect(() => {
    if (display && gltfUuid) {
      ...
      const nepNoc1 = md.getObjectByName('G-NEP-NOC-1');
      const nepNoc2 = md.getObjectByName('G-NEP-NOC-2');
      ...
      nepNoc1 && settingNepNoc1(nepNoc1);
      nepNoc2 && settingNepNoc2(nepNoc2);
