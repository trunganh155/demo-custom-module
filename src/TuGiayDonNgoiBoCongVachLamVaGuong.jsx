...
function App4() {
  ...
  const settingHau = (hau) => {
    hau.position.z = (bct ? DDTBia + rBo + truHaoBoCong : DDTBia) * -1;

    hau.position.y = caoChan + DDTBia;
  ...
  const settingVachGuong = (vachGuong) => {
    vachGuong.position.z = 0 * -1;

    vachGuong.position.y = height - caoNem;

    vachGuong.position.x = 0;
    ...
    vachGuong.scale.set(1, 1, 1);
    let boundingBoxHau = new THREE.Box3().setFromObject(vachGuong);
    const sizeHau = new THREE.Vector3();
    boundingBoxHau.getSize(sizeHau);

    vachGuong.scale.x = lenX / sizeHau.x;
    vachGuong.scale.y = lenY / sizeHau.y;
    vachGuong.scale.z = lenZ / sizeHau.z;
  };

  const settingGuong = (guong) => {
    guong.position.z = 0.01 * -1;

    guong.position.y = height + 0.01;

    guong.position.x = DDTBia;

    const lenZ = width - 0.01 * 2;

    const lenX = 0.008;

    const lenY = caoVachLam - 0.01 * 2;

    guong.scale.set(1, 1, 1);
    let boundingBoxHau = new THREE.Box3().setFromObject(guong);
    const sizeHau = new THREE.Vector3();
    boundingBoxHau.getSize(sizeHau);

    guong.scale.x = lenX / sizeHau.x;
    guong.scale.y = lenY / sizeHau.y;
    guong.scale.z = lenZ / sizeHau.z;
  };
  //KHOI CHINH
  ...
  useEffect(() => {
    ...
    glftLoader.load(
      '/glb/DON-NGOI-BO-CONG-VACH-LAM-VA-GUONG-UV.glb',
      (gltfScene) => {
  ...
  useEffect(() => {
    if (display && gltfUuid) {
      ...
      const vachGuong = md.getObjectByName('VACH-GUONG');
      const guong = md.getObjectByName('GUONG');
      ...
      noc && settingNoc(noc);
      hau && settingHau(hau);
      vachGuong && settingVachGuong(vachGuong);
      chanTruoc && settingChanTruoc(chanTruoc);
      guong && settingGuong(guong);
      //KHOI CHINH
    ...
    <div>
      <div className="container-option">
        ...
        <div>
          ...
          <br />
          <label className="label" htmlFor="caoVachLam">
            Cao vách lam(mm):
          </label>
          <input
            className="input"
            ...
            name="caoVachLam"
            id="caoVachLam"
            defaultValue={caoVachLam * 1000}
            onChange={(e) => {
              setCaoVachLam(Number(e.target.value) / 1000);
            }}
          />

          <br />
          <label className="label" htmlFor="rBo">
            Bán kính bo cong (mm):
          </label>
          <input
            className="input"
