...
function App3() {
  ...
  const settingXanNgang1 = (xNgang1) => {
    ...
    xNgang1.position.x = truHaoGoc + DDTBia;
    xNgang1.position.y = (height - caoChan - caoXTruoc) / 2 + caoChan;

    const lenZ = depth - DDTHau;
    const lenX = rongK1 - 2 * DDTBia - truHaoGoc;
    const lenY = DDTBia;
  ...
  useEffect(() => {
    ...
    glftLoader.load('/glb/TBDGL2-FULL.glb', (gltfScene) => {
  ...
  useEffect(() => {
    if (display && gltfUuid) {
      ...
      const nepTuGoc = md.getObjectByName('NEP-TU-GOC');
