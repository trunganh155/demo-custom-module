...
function App() {
  ...
  const settingMatDa1 = (matDa1) => {
    matDa1.position.z = (width - rBo - 0.003) * -1;

    matDa1.position.y = height;

    matDa1.position.x = depth - rBo;

    const lenZ = rBo + 0.003;

    const lenX = rBo + 0.003;
  ...
  const settingMatDa2 = (matDa2) => {
    ...
    matDa2.position.x = depth - rBo;

    const lenZ = width - rBo - 0.003;
  ...
  useEffect(() => {
    ...
    glftLoader.load('/glb/KHOIBOCONG-PHAI-UV-CoMatDa.glb', (gltfScene) => {
