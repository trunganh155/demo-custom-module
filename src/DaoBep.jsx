...
function App3() {
  ...
  const settingChanTruoc = (chanTruoc) => {
    chanTruoc.position.z = DDTBia * -1;

    chanTruoc.position.x = depth - DDTBia;

    chanTruoc.position.y = 0;
    ...
    chanTruoc.scale.set(1, 1, 1);
    let boundingBoxChanTruoc = new THREE.Box3().setFromObject(chanTruoc);
    const sizeChanTruoc = new THREE.Vector3();
    boundingBoxChanTruoc.getSize(sizeChanTruoc);

    chanTruoc.scale.x = lenX / sizeChanTruoc.x;
    chanTruoc.scale.y = lenY / sizeChanTruoc.y;
    chanTruoc.scale.z = lenZ / sizeChanTruoc.z;
  };

  const settingChanSau = (chanSau) => {
    chanSau.position.z = DDTBia * -1;

    chanSau.position.x = depth / 2;

    chanSau.position.y = 0;
    ...
    chanSau.scale.set(1, 1, 1);
    let boundingBoxChanTruoc = new THREE.Box3().setFromObject(chanSau);
    const sizeChanTruoc = new THREE.Vector3();
    boundingBoxChanTruoc.getSize(sizeChanTruoc);

    chanSau.scale.x = lenX / sizeChanTruoc.x;
    chanSau.scale.y = lenY / sizeChanTruoc.y;
    chanSau.scale.z = lenZ / sizeChanTruoc.z;
  };
  ...
  const settingXuongSau = (xSau) => {
    xSau.position.z = DDTBia * -1;

    xSau.position.x = DDTHau;

    xSau.position.y = height - caoXSau;
  ...
  useEffect(() => {
    ...
    glftLoader.load('/glb/DAO-BEP.glb', (gltfScene) => {
